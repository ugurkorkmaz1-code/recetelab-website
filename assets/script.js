...
$('#contactForm')?.addEventListener('submit',function(e){e.preventDefault();if(!this.reportValidity())return;const d=new FormData(this),rows=[`ReçeteLab consultation request (${RECETELAB.lang})`,`Name: ${d.get('name')||'-'}`,`Company: ${d.get('company')||'-'}`,`Phone: ${d.get('phone')||'-'}`,`Email: ${d.get('email')||'-'}`,`Service: ${d.get('service')||'-'}`,`Project: ${d.get('message')||'-'}`];const waUrl=`https://wa.me/905537718776?text=${encodeURIComponent(rows.join('\n'))}`;trackWhatsApp('contact_form',waUrl,'contact_form_submit');window.location.href=waUrl;});
