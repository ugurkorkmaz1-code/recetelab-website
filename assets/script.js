...
(()=>{
  ...
  if(!document.querySelector(`script[data-recetelab-ga4="${GA_MEASUREMENT_ID}"]`)){
    ...
    s.dataset.recetelabGa4=GA_MEASUREMENT_ID;
    document.head.appendChild(s);
  }
})();
const trackWhatsApp=(source,linkUrl='',label='')=>{
  if(typeof window.gtag!=='function')return;
  gtag('event','whatsapp_click',{
    source,
    link_url:linkUrl,
    element_text:(label||'').trim().slice(0,100),
    page_location:location.href,
    page_path:location.pathname,
    page_language:document.documentElement.lang||window.RECETELAB?.lang||''
  });
};
document.addEventListener('click',e=>{
  ...
  if(!a)return;
  const href=a.href||'';
  if(/(?:wa\.me|whatsapp\.com)/i.test(href))trackWhatsApp('whatsapp_link',href,a.textContent||'');
});
